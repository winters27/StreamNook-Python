...
# === and configuration of the full Streamlink + MPC-HC suite.
# ===================================================================
...
$global:urls = @{
  'mpc-hc' = @{
    Type    = "github-latest"
    Repo    = "clsid2/mpc-hc"
    Pattern = "MPC-HC*.x64.exe"  # matches MPC-HC.2.5.5.x64.exe
  }
  'streamlink' = @{
    Type    = "github-latest"
...
$global:stepInstructions = @{
    1 = @{
        ...
        Text  = "This wizard will guide you through manually installing and configuring your complete Streamlink setup.`n`nClick 'Next' to begin."
    }
    2 = @{
        Title = "Step 1: Install MPC-HC"
        Text  = "1. Click the 'Download' button below to automatically download the latest version.`n`n2. Run the installer and use the default location (C:\Program Files\MPC-HC).`n`n3. Click 'Next' when done."
    }
    3 = @{
        Title = "Step 2: Install Streamlink"
    ...
    4 = @{
        Title = "Step 3: Install Streamlink Twitch GUI"
    ...
    7 = @{
        Title = "Step 6: Configure Player"
        Text  = "Open Streamlink Twitch GUI and go to Settings (gear icon).`n`n1. Go to the **Player** tab.`n`n2. For **'Player path'**, set it to:`nC:\Program Files\MPC-HC\mpc-hc64.exe`n`n3. For **'Player arguments'**, click the 'Copy Args' button below and paste the text in the box."
    }
    8 = @{
        Title = "Step 7: Configure Streaming (Ads)"
...
$buttonDownload.Size = New-Object System.Drawing.Size(180, 40)
$buttonDownload.Location = New-Object System.Drawing.Point(120, 10)
...
function Load-Step {
    ...
    switch ($step) {
        1 { $labelSubtitle.Text = "Let's get your streaming setup configured" }
        2 { $labelSubtitle.Text = "Media Player Classic - Home Cinema" }
        3 { $labelSubtitle.Text = "Core streaming engine" }
        4 { $labelSubtitle.Text = "User-friendly interface for Twitch" }
        5 { $labelSubtitle.Text = "Enhanced chat with 7TV emotes" }
        6 { $labelSubtitle.Text = "Ad-blocking plugin for Twitch" }
        7 { $labelSubtitle.Text = "Connect MPC-HC to Streamlink" }
        8 { $labelSubtitle.Text = "Enable ad-free streaming" }
        9 { $labelSubtitle.Text = "Setup complete!" }
    }
    ...
    switch ($step) {
        2 { # MPC-HC
            ...
            $buttonDownload.Text = "🡇 Download MPC-HC"
            $buttonDownload.Tag = "mpc-hc"
        }
        3 { # Streamlink
            $buttonDownload.Visible = $true
        ...
        7 { # Configure Player
            ...
            $buttonAction.Location = New-Object System.Drawing.Point(220, 10)
        }
    ...
    if ($this.Tag) {
        ...
        try {
            ...
            elseif ($urlInfo.Type -eq "github-latest") {
                ...
                # Determine if the resolved URL points directly to a file (.exe/.py), even with querystrings
                $leaf = Split-Path -Leaf ($downloadUrl -split '\?' | Select-Object -First 1)
                $ext  = [IO.Path]::GetExtension($leaf)

                if ($ext -and @(".exe", ".py") -contains $ext.ToLower()) {
                    $fileName = $leaf
                    ...
                    $saveDialog.Filter = if ($ext.ToLower() -eq ".exe") { "Executable files (*.exe)|*.exe" } else { "Python files (*.py)|*.py|All files (*.*)|*.*" }
...
$buttonAction_Click = {
    switch ($buttonAction.Tag) {
        ...
        "copy_player_args" {
            $playerArgs = '"{filename}" /play /close /new /fixedsize 1075,605 /viewpreset 1'
            Set-Clipboard -Value $playerArgs
            [System.Windows.Forms.MessageBox]::Show(
                "Player arguments copied to clipboard!`n`nPaste them into the Player Arguments field in Streamlink Twitch GUI settings.", 
                "✓ Copied Successfully", 
                "OK", 
                "Information"
            )
